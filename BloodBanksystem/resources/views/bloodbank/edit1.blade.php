...
<div class="card customcard2">
      <h2 class="card-title customcard">Edit Inventory</h2>
          ...
          <form action="{{route('inventory.update', [$data->id])  }}" method="POST">
            ...
            <option value="{{$data->blood_type}}" selected>{{$data->blood_type}}</option>
            ...
            <div class="form-group">
              ...
              <input type="number" class="form-control" value="{{$data->quantity}}" id="quantity" name="quantity" required>
            </div>
            ...
            <button type="submit" class="btn btn-primary btn-block custombtn">Update</button>
          </form>
        </div>
      </div> 
</div> 
